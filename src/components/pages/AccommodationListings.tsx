...
const statusStyles: Record<string, { tint: string; icon: ComponentType<{ className?: string }> }> = {
	Verified: { tint: 'bg-emerald-50 text-emerald-600', icon: BadgeCheck },
	'Pending review': { tint: 'bg-amber-50 text-amber-600', icon: Clock },
...
function AccommodationListings() {
	...
	const filteredListings = listings.filter((listing) => {
		...
		const matchesQuery = listing.title.toLowerCase().includes(query.toLowerCase()) || listing.area.toLowerCase().includes(query.toLowerCase())
		...
		<div className="flex min-h-screen bg-slate-50">
			...
			<main className="flex-1 px-4 py-6 sm:px-6 lg:px-10">
				<div className="flex flex-wrap items-center justify-between gap-4">
					<div>
						<h1 className="text-2xl font-black tracking-[-0.04em] text-slate-950 sm:text-3xl">My Listings</h1>
				...
				<p className="mt-5 text-sm text-slate-500">
					<span className="font-bold text-slate-950">{filteredListings.length}</span> of {listings.length} listings
				</p>

				<div className="mt-4 grid gap-5 md:grid-cols-2 xl:grid-cols-3">
					{filteredListings.map((listing) => {
						const style = statusStyles[listing.status]
						...
						return (
							...
							>
								<div className={`relative h-40 bg-gradient-to-br ${listing.gradient}`}>
									<span className={`absolute left-3 top-3 inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs font-bold shadow-sm ${style.tint}`}>
				...
				{filteredListings.length === 0 && (
					<div className="mt-10 rounded-2xl border border-dashed border-slate-200 bg-white p-10 text-center">
						<p className="text-sm font-semibold text-slate-600">No listings match your filters.</p>
						<p className="mt-1 text-sm text-slate-400">Try a different status or search term.</p>
					</div>
				)}
