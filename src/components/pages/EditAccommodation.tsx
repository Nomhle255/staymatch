...
function EditAccommodation() {
	...
	const reverseGeocodeArea = async (
		...
		const label = [neighbourhood, town]
			.filter(Boolean)
			.join(', ')

		if (!label)
			throw new Error(
				'Could not determine an area name for this location.'
			)
	...
	const confirmLocation = async () => {
		...
		try {
			const resolvedArea = await reverseGeocodeArea(
				latitude,
				longitude
			)
			...
			setLocationError(
				'Could not determine the area for this location. Please try a different spot on the map.'
			)
	...
	const handleSubmit = async (
		...
		if (
			...
			setError(
				'Please select and confirm the property location before saving.'
			)
			return
		}
		...
		try {
			...
			setTimeout(() => {
				router.push(
					`/landlord/accommodationdetails?id=${accommodationId}`
				)
			}, 1000)
		} catch (err) {
			setError(
				...
				>
					<div className="space-y-6">
						...
						<section className="rounded-[1.75rem] border border-slate-200/70 bg-white p-6 shadow-sm">
							<div className="flex items-start justify-between gap-4">
								<div>
									...
									<p className="mt-1 text-sm leading-6 text-slate-500">
										Move the pin if the location
										needs updating. The area name
							...
							<p className="mt-3 text-xs text-slate-400">
								...
								or drag the marker to adjust it.
							</p>
					...
					<div className="space-y-6">
						...
						<section className="rounded-[1.75rem] border border-slate-200/70 bg-white p-6 shadow-sm">
							...
							<p className="mt-1 text-sm text-slate-500">
								Add or remove photos of the room, common
								areas, and exterior.
							</p>
									...
									{photos.map((photo, index) => (
										...
										>
											...
											<button
												...
												className="absolute right-2 top-2 flex h-7 w-7 items-center justify-center rounded-full bg-slate-950/70 text-white opacity-0 transition hover:bg-rose-600 group-hover:opacity-100"
											...
											<div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-slate-950/70 to-transparent px-2 pb-2 pt-6">
						...
						<section className="rounded-[1.75rem] border border-slate-200/70 bg-white p-6 shadow-sm">
							...
							<p className="mt-1 text-sm leading-6 text-slate-500">
								Changes are saved immediately and reflected
								on your listing right away.
							</p>
